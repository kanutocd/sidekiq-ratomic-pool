...
server_log="$(mktemp -t sidekiq-ratomic-pool-smoke.XXXXXX.log)"
client_log="$(mktemp -t sidekiq-ratomic-pool-smoke-client.XXXXXX.log)"
server_pid=''
cleanup() {
...
bundle exec sidekiq -r ./server.rb -q smoke -c "${SIDEKIQ_CONCURRENCY:-4}" >"$server_log" 2>&1 &
server_pid=$!

printf '\nCPU cores visible to the smoke test: '
nproc 2>/dev/null || getconf _NPROCESSORS_ONLN
printf 'Sidekiq server PID: %s\n' "$server_pid"

bundle exec ruby ./client.rb >"$client_log" 2>&1 &
client_pid=$!
