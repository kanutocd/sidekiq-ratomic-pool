...
server_log="$(mktemp -t sidekiq-ratomic-pool-benchmark.XXXXXX.log)"
client_log="$(mktemp -t sidekiq-ratomic-pool-benchmark-client.XXXXXX.log)"
server_pid=''
cleanup() {
...
bundle exec sidekiq -r ./benchmark/server.rb -q benchmark -c "${BENCHMARK_CONCURRENCY:-4}" >"$server_log" 2>&1 &
server_pid=$!
